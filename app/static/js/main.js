    ...
    try {
        ...
        switch(plotType) {
            ...
            case '/plot_monitored_area':
                if (!formData.get('rect_min_x'||'rect_max_x'||'rect_min_y'||'rect_max_y')) {
                    throw new Error('Parâmetros de Área obrigatórios!');
                }
                else if (!formData.get('category')) {
                    throw new Error('Selecione uma categoria!');
        ...
        if(widthCm < 1 || widthCm > 50 || heightCm < 1 || heightCm > 50){
            ...
            if (data.image) {
                ...
                metricsContainer.innerHTML = `
                    <div class="metric-section">
                        ...
                        ${buildMetricsHTML(data.metrics.summary)}
                    </div>
                `;
...
function generateMetricsOnly() {
    ...
    try {
        ...
        .catch(async (error) => {
            if (error instanceof Response) {
                const err = await error.json();
                alert(`Erro ${error.status}: ${err.message}`);
            } else {
                alert(error.message);
            }
        })
        .finally(() => {
            spinner.style.display = 'none';
            buttonText.textContent = 'Gerar Apenas Métricas';
            button.disabled = false;
        });
    } catch (error) {
        alert(error.message);
